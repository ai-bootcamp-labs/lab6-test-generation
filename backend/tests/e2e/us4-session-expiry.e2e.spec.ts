...
describe('Story 4: Session expiry & logout (E2E)', () => {
  it('Scenario 1 — explicit logout invalidates the calling session immediately', async () => {
    const { sessionCookie, csrfCookie, csrfValue } = await seedAndLogin('us4-1@example.com');
  ...
  it('Scenario 2 — logout of session A leaves a parallel session B unaffected', async () => {
    const a = await seedAndLogin('us4-2@example.com');
    const b = await seedAndLogin('us4-2@example.com');
    const out = await request(app)
      .post('/auth/logout')
      .set('Cookie', [a.sessionCookie, a.csrfCookie].join('; '))
      .set('x-csrf-token', a.csrfValue);
    expect(out.status).toBe(204);
    const aProbe = await request(app).get('/auth/session').set('Cookie', a.sessionCookie);
    const bProbe = await request(app).get('/auth/session').set('Cookie', b.sessionCookie);
    expect(aProbe.status).toBe(401);
    expect(bProbe.status).toBe(200);
  });

  it('Scenario 3 — logout without CSRF header is rejected with 403', async () => {
    const { sessionCookie, csrfCookie } = await seedAndLogin('us4-3@example.com');
    const out = await request(app)
      .post('/auth/logout')
      .set('Cookie', [sessionCookie, csrfCookie].join('; '));
  ...
  it('Scenario 4 — logout without any session cookie returns 401', async () => {
